...
\fancyhead[L]{\small\textbf{AIGGPA --- Rural Development Department}}\fancyhead[R]{\small ID: \_\_\_\_\_\_\_\_}
...
{\large\bfseries Rural Development Department}\\[3pt]
...
\noindent\textbf{Key Digital Tools:} NREGASoft / NMMS App, e-Gram Swaraj, PMAY-G Portal, SBM-G Portal, Panchayat Darpan, PFMS
...
\rule{\textwidth}{0.5pt}
\newpage
\noindent{\large\bfseries Section I: Rural Development Department --- Specific Questions}\vspace{4pt}

\noindent\begin{tabularx}{\textwidth}{@{}L{0.6cm} X L{5.5cm}@{}}
Q40 & Which Rural Development tools are you aware of? & {\Large$\square$} NREGASoft/NMMS {\Large$\square$} e-Gram Swaraj {\Large$\square$} PMAY-G {\Large$\square$} SBM-G {\Large$\square$} Panchayat Darpan {\Large$\square$} PFMS {\Large$\square$} None \\[3pt]
Q41 & How difficult is managing and updating data across multiple portals (NREGASoft, e-Gram Swaraj, PMAY-G) at the same time? & \likert{} {\scriptsize(1=Very Easy, 5=Very Difficult)} \\[3pt]
Q42 & Rate internet connectivity at block-level or panchayat-level offices for uploading data: & \likert{} {\scriptsize(1=No connectivity, 5=Excellent)} \\[3pt]
Q43 & Has the NMMS app improved accuracy of MGNREGA attendance and worksite verification? & \likert \\[3pt]
Q44 & How much of your working day is spent entering data into portals? & {\Large$\square$} Less than 1 hr {\Large$\square$} 1--2 hrs {\Large$\square$} 2--4 hrs {\Large$\square$} 4+ hrs {\Large$\square$} Almost the entire day \\[3pt]
Q45 & When preparing MGNREGA muster rolls and wage lists, which steps are digital and which are on paper? & Digitally: \rule{6cm}{0.4pt}\\[2pt] && On paper: \rule{6cm}{0.4pt} \\[3pt]
Q46 & Have you personally used geo-tagging (GPS photo) for PMAY-G house verification or MGNREGA worksite? & {\Large$\square$} Yes, regularly {\Large$\square$} Yes, a few times {\Large$\square$} No, someone else does it {\Large$\square$} No, never \\[3pt]
Q47 & When e-Gram Swaraj or NREGASoft is down or slow, what do you do? & {\Large$\square$} Wait and retry {\Large$\square$} Record on paper, upload later {\Large$\square$} Go to block office {\Large$\square$} Ask someone else {\Large$\square$} Skip the entry \\[3pt]
Q48 & As a GRS / Panchayat Secretary, what is the single biggest problem with portal-based reporting? & \rule{\linewidth}{0.4pt}\\[2pt] && \rule{\linewidth}{0.4pt} \\[3pt]
Q49 & Does the BDO / CEO review the data you upload, or is it only checked at district level? & {\Large$\square$} BDO reviews regularly {\Large$\square$} BDO checks sometimes {\Large$\square$} Only district reviews {\Large$\square$} Nobody reviews {\Large$\square$} Don't know \\[3pt]
\end{tabularx}
...
\textbf{Total: 39 (common) + 10 (Rural Development) + 8 (role) = 57 questions}\\[8pt]
