...
\fancyhead[L]{\small\textbf{AIGGPA --- Revenue Department}}\fancyhead[R]{\small ID: \_\_\_\_\_\_\_\_}
...
{\large\bfseries Revenue Department}\\[3pt]
...
\noindent\textbf{Key Digital Tools:} MP Bhulekh / WebGIS 2.0, RCMS, SAARA, SAMPADA 2.0, e-Court / e-Filing
...
\rule{\textwidth}{0.5pt}
\newpage
\noindent{\large\bfseries Section I: Revenue Department --- Specific Questions}\vspace{4pt}

\noindent\begin{tabularx}{\textwidth}{@{}L{0.6cm} X L{5.5cm}@{}}
Q40 & Which Revenue digital tools are you aware of? & {\Large$\square$} Bhulekh/WebGIS {\Large$\square$} RCMS {\Large$\square$} SAARA {\Large$\square$} SAMPADA 2.0 {\Large$\square$} e-Court/e-Filing {\Large$\square$} None \\[3pt]
Q41 & Has Bhulekh/WebGIS improved speed and accuracy of land record verification in your office? & \likert \\[3pt]
Q42 & How difficult is RCMS to use for tracking revenue cases? & \likert{} {\scriptsize(1=Very Easy, 5=Very Difficult)} \\[3pt]
Q43 & What percentage of land records or revenue cases in your office still need physical paper files? & {\Large$\square$} 0--20\% {\Large$\square$} 21--40\% {\Large$\square$} 41--60\% {\Large$\square$} 61--80\% {\Large$\square$} 81--100\% \\[3pt]
Q44 & How often do citizens visit expecting services through digital tools (e.g., online land record, e-stamp)? & \likert{} {\scriptsize(1=Never, 5=Multiple times daily)} \\[3pt]
Q45 & When you process a mutation (dakhil-kharij) case, which steps do you complete digitally and which on paper? & Digitally: \rule{6cm}{0.4pt}\\[2pt] && On paper: \rule{6cm}{0.4pt} \\[3pt]
Q46 & Has SAMPADA 2.0 reduced the time for property registration compared to the earlier manual process? & {\Large$\square$} Much faster {\Large$\square$} Somewhat faster {\Large$\square$} No change {\Large$\square$} Slower now {\Large$\square$} Don't use it \\[3pt]
Q47 & How often does the Bhulekh/WebGIS portal show errors, timeouts, or incorrect map data? & {\Large$\square$} Never {\Large$\square$} Rarely {\Large$\square$} Sometimes {\Large$\square$} Often {\Large$\square$} Almost always \\[3pt]
Q48 & As a Patwari/RI/Tehsildar, what is the biggest obstacle to completing Khasra/B-1 updates digitally? & \rule{\linewidth}{0.4pt}\\[2pt] && \rule{\linewidth}{0.4pt} \\[3pt]
Q49 & Do you receive different digital tasks based on your post (e.g., Patwari does data entry, Tehsildar does approval)? & {\Large$\square$} Yes, clearly separated {\Large$\square$} Somewhat {\Large$\square$} No, everyone does the same {\Large$\square$} Don't know \\[3pt]
\end{tabularx}
...
\textbf{Total: 39 (common) + 10 (Revenue) + 8 (role) = 57 questions}\\[8pt]
